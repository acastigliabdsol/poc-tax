...
async fn main() -> Result<(), GooseError> {
    ...
    let rpc_addr = env::var("RPC_ADDR").unwrap_or_else(|_| "127.0.0.1:50051".to_string());
    
    let _metrics = GooseAttack::initialize()?
        // Set a dummy host - we use RPC_ADDR env var for actual connection
        .set_default(GooseDefault::Host, format!("http://{}", rpc_addr).as_str())?
        .register_scenario(
